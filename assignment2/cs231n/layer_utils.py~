...
def affine_layer_relu_forward(x,w,b,gamma,beta,ln_param):
    ...
    '''
    a,fc_cache=affine_forward(x,w,b)
    la,la_cache=layernorm_forward(a,gamma,beta,ln_param)
    out,relu_cache=relu_forward(la)
    cache=(fc_cache,la_cache,relu_cache)
    return out,cache
...
def affine_layer_relu_backward(dout,cache):
    ...
    dout.shape=N*D 
    '''
    fc_cache,la_cache,relu_cache=cache
    dla=relu_backward(dout,relu_cache)
    da,dgamma,dbeta=layernorm_backward(dla,la_cache)
    dx,dw,db=affine_backward(da,fc_cache)
    return dx,dw,db,dgamma,dbeta
...
def conv_bn_relu_pool_forward(x,w,b,gamma,beta,conv_param,bn_param,pool_param):
    ...
    out,pool_cache=max_pool_forward_naive(relu_cache,pool_param)
    cache=(conv_cache,bn_cache,relu_cache,pool_cache)
    return out,cache

def conv_bn_relu_pool_backward(dout,cache):
    conv_cache,bn_cache,relu_cache,pool_cache=cache
    dpool=max_pool_backward_naive(dout,pool_cache)
