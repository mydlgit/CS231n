        ...
        for i in range(self.num_layers):
            ...
            if i ==self.num_layers -1 :
                ...
                if self.use_batchnorm:
                    ...
                    gamma,beta=self.params['gamma' + str(i+1)],self.params['beta' + str(i+1)] 
                    hidden_layers[i+1],caches[i]=affine_layer_relu_forward(hidden_layers[i],w,b,gamma,beta,self.ln_params[i])
                else:                    
        ...
        for i in range(self.num_layers, 0, -1):
            if i == self.num_layers:
                ...
                if self.use_batchnorm:
                    ...
                    dx,dw,db,dgamma,dbeta=affine_layer_relu_backward(dhiddens[i],caches[i-1])
                    dhiddens[i-1], grads['W' + str(i)], grads['b' + str(i)] = dx, dw, db
                    grads['gamma'+str(i)], grads['beta' + str(i)] = dgamma, dbeta 
                else: 
